...
/*   Updated: 2020/10/04 12:01:35 by qtamaril         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	*check_path(char *path, char *cmd_name)
{
	char	*full_path;
	int		fd;
...
char	*parse_path(char **cmd, t_list *env)
{
	...
	while (tmp)
	{
		if (!ft_strcmp(((t_env*)tmp->content)->name, "PATH"))
		{
			...
			{
				if ((true_path = check_path(splitted_path[i++], \
					ft_strjoin("/", cmd[0]))))
				return (true_path);
			}
...
void	run_command(char **cmd, t_list *env)
{
	...
	if (!(true_path = parse_path(cmd, env)))
		exit(0); // с каким значением? (если не нашел команду)
	pid = fork();
	if (pid == -1)
